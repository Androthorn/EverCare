...
criaChatP :: Int -> Int -> String -> IO ()
criaChatP idPaciente idMedico mensagem = read (intercalate ";" ([show (idPaciente), show (idMedico), mensagem])) :: Chat
   
-- Função para enviar mensagem de paciente para médico
enviarMensagemParaMedico :: Int -> String -> IO ()
enviarMensagemParaMedico idChat mensagem = 

-- Função para listar todas as mensagens recebidas por um paciente
listarChats :: Int -> [Chat.Chat] -> IO ()
listarChats idPaciente chats = filter (\chat -> Chat.idPaciente chat == idPaciente) chats

-- Função para ler mensagens de um médico específico
lerMensagensDeMedico :: Int -> [Chat.Chat] -> IO ()
